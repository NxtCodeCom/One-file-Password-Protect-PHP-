...
extends config
{
					...
					$("#login").click(
						function() {
							...
							if ($("#soy_password").attr("value") == "") {
								$("#soy_password").addClass("alert").delay(3000).queue(function(next) {
    ...
    border: 1px solid red;
      ...
      margin: 10% auto 0 !important;">
        ...
        <? if ($error_msg) {
				$error_msg =  $error_msg . "This is your attempt :" . $attempt;
			} ?>
        ...
        <? } else {
			$ATTEMPT_COOKIE =	parent::ATTEMPT_COOKIE;
	...
	public function Login()
	{
		...
		if (isset($_POST['soy_password'])) {
			$username = trim($_POST['soy_login']);
			/** user provided username */
			$pass = trim($_POST['soy_password']);
			/** user provided password */
			...
			if ((parent::PASSWORD != $pass)) {
				...
				$_SESSION['start'] = time();
				/** to register the start time of the session  */
